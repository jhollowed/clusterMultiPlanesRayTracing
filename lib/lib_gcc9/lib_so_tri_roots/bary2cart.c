...
void mapping_triangles_arrays(double *ys1_array, double *ys2_array, int ngals, double *xgrids1,double *xgrids2,double *lgrids1,double *lgrids2, int nc, double *xr1, double *xr2) {
	int i, j, nonzero;

	for (i = 0; i < ngals; i++) {
		/*printf("---------------------%d\n", i);*/
		double *xroots_out = (double *)calloc(10, sizeof(double));
		double *ys_tmp = (double *)calloc(2, sizeof(double));
        ys_tmp[0] = ys1_array[i];
        ys_tmp[1] = ys2_array[i];

		mapping_triangles(ys_tmp,xgrids1,xgrids2,lgrids1,lgrids2,nc,xroots_out);
...
void mapping_triangles_arrays_omp(double *ys1_array, double *ys2_array, int ngals, double *xgrids1,double *xgrids2,double *lgrids1,double *lgrids2, int nc, double *xr1, double *xr2) {

	int i, j, nonzero;
#pragma omp parallel num_threads(8)	\
	shared(ys1_array, ys2_array, ngals, xgrids1, xgrids2, lgrids1, lgrids2, nc, xr1, xr2) \
	private(i, j, nonzero)
	{
		#pragma omp for schedule(dynamic,16)
		for (i = 0; i < ngals; i++) {
			double *ys_tmp = (double *)calloc(2, sizeof(double));
			double *xr_tmp = (double *)calloc(10, sizeof(double));

    	    ys_tmp[0] = ys1_array[i];
    	    ys_tmp[1] = ys2_array[i];

			mapping_triangles(ys_tmp,xgrids1,xgrids2,lgrids1,lgrids2,nc,xr_tmp);
