...
class raytracer:
    def __init__(self, halo_dir, out_dir, zs=[1.0], seed=None):
        """
        Class for running ray-tracing routines on simple NFW particle distributions as generated by 
        a make_siomple_halo.simple_halo() instance. Output is various lensing maps (see documentation
        for create_grid_maps.py, make_lensing_mocks.py, and raytrace.py in the directory above).
        
        Parameters
        ----------
        halo_dir : str
            Locaton of particle data as output from make_simple_halo.simple_halo.output_particles()
        out_dir : str
            Output location for lensing maps
        zs : flost list; optonal
            List of source redshifts to include in raytracing. Defaults to [1.0], a single source plane 
            at z=1
        seed : float, optional
            Random seed to pass to make_lensing_mocks for placement of interpolation points on lensing
            maps. Default value of None gives stochastic results.
        
        Methods
        -------
        halo_raytrace():
            Uses the modules of this package to generate lensing maps and interpolated mocks for the
            input matter distribution
        vis_outputs():
            Draws figures displaying the defelction planes, and vector shear field overlain on the 
            scalar convergence field
        """
    ...
    def vis_outputs(self):
        '''
        Draws figures displaying the defelction planes, and vector shear field overlain on the 
        scalar convergence field given by the output of halo_raytrace
        '''
        
        halo_dir = self.halo_dir
        lensing_dir = self.out_dir
        zs = self.zs
...
# --- example usage ---
# halo_raytrace(halo_dir, lesning_dir, zs = [1, 1.1])
# vis_outputs(lensing_dir)
